...
class UserTierVerifierTest {
    ...
    fun `test isProUser with no tier blocks`() = runBlocking {
        // Arrange
    ...
    fun `test isProUser with no valid tier blocks`() = runBlocking {
        // Arrange
        val userPublicKey = ByteArray(32) { 1 }
        val currentTime = System.currentTimeMillis()
        val expiredBlock = mockk<UserTierBlock> {
    ...
    fun `test isProUser with valid PRO tier block`() = runBlocking {
        // Arrange
        val userPublicKey = ByteArray(32) { 1 }
        val currentTime = System.currentTimeMillis()
        val validProBlock = mockk<UserTierBlock> {
            every { validFrom } returns currentTime - 1000
    ...
    fun `test isProUser with valid BASIC tier block`() = runBlocking {
        // Arrange
        val userPublicKey = ByteArray(32) { 1 }
        val currentTime = System.currentTimeMillis()
        val validBasicBlock = mockk<UserTierBlock> {
            every { validFrom } returns currentTime - 1000
            every { validUntil } returns currentTime + 1000
    ...
    fun `test isProUser with multiple valid blocks`() = runBlocking {
        // Arrange
        val userPublicKey = ByteArray(32) { 1 }
        val currentTime = System.currentTimeMillis()
        val olderProBlock = mockk<UserTierBlock> {
            every { validFrom } returns currentTime - 2000
            every { validUntil } returns currentTime + 1000
            every { tier } returns "PRO"
        }
        val newerBasicBlock = mockk<UserTierBlock> {
            every { validFrom } returns currentTime - 1000
            every { validUntil } returns currentTime + 1000
            every { tier } returns "BASIC"
        }
    ...
    fun `test isProUser with null validUntil`() = runBlocking {
        // Arrange
        val userPublicKey = ByteArray(32) { 1 }
        val currentTime = System.currentTimeMillis()
        val infiniteProBlock = mockk<UserTierBlock> {
            every { validFrom } returns currentTime - 1000
