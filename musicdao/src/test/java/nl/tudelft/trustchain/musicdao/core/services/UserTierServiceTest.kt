...
class UserTierServiceTest {
    ...
    @Test
    fun `test upgradeToPro fails when wallet balance is null`() =
        runBlocking {
    ...
    @Test
    fun `test upgradeToPro fails when insufficient balance`() =
        runBlocking {
    ...
    @Test
    fun `test upgradeToPro fails when payment fails`() =
        runBlocking {
    ...
    @Test
    fun `test upgradeToPro fails when block creation fails`() =
        runBlocking {
    ...
    @Test
    fun `test upgradeToPro succeeds with valid parameters`() =
        runBlocking {
            val balance = Coin.parseCoin("0.2")
    ...
    @Test
    fun `test upgradeToUltimate fails when wallet balance is null`() =
        runBlocking {
            every { bitcoinWalletViewModel.confirmedBalance.value } returns null

            val result =
                service.upgradeToUltimate(
                    userId = "test_user",
                    durationMonths = 1,
                    bitcoinWalletViewModel = bitcoinWalletViewModel
                )
    ...
    @Test
    fun `test upgradeToUltimate fails when block creation fails`() =
        runBlocking {
            val balance = Coin.parseCoin("0.2")
            every { bitcoinWalletViewModel.confirmedBalance.value } returns balance
            coEvery {
    ...
    @Test
    fun `test upgradeToUltimate succeeds with valid parameters`() =
        runBlocking {
            val balance = Coin.parseCoin("0.2")
            val block = mockk<TrustChainBlock>()
    ...
    @Test
    fun `test downgradeToBasic fails when block creation fails`() =
        runBlocking {
            coEvery {
                userTierBlockRepository.create(
                    userId = "test_user",
    ...
    @Test
    fun `test downgradeToBasic succeeds with valid parameters`() =
        runBlocking {
            val block = mockk<TrustChainBlock>()
            coEvery {
                userTierBlockRepository.create(
